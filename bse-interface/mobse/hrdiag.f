...
*
c      mass0 = mass
c      if(mass0.gt.300.d0.and.saveflag.eqv..true.)then
c         WRITE(99,*)' MASS EXCEEDED in hrdiag.f',mass0
c         saveflag = .false.
c      endif
*
      mt0 = mt
c      if(mt0.gt.300.d0.and.saveflag.eqv..true.)then
c         WRITE(99,*)' MASS EXCEEDED in hrdiag.f',mt
c         saveflag = .false.
c      endif
***
*
...
c New coefficient from Nbody6??
c         mcmax = MAX(MAX(mch,0.773d0*mcbagb-0.35d0),1.02d0*mcx)
*
...
***
            kw = 13
            aj = 0.d0
            mt = 6.6667d0*(sqrt(1.d0 + 0.3d0*1.38d0) - 1.d0)
            ECS = 1
* to mantain the bimodal distribution
            ffb = 0.0d0
*
...
*
c      if(mass.gt.150.0d0)then
c         mass = mass0
c      endif
c      if(mt.gt.150.0d0)then
c        mt = mt0
c      endif
*
      if(kw.eq.15)then
