...
petar.select --require agama --optional mpi,omp,avx512,avx2
...
dt_rec=$(petar.find.dt -a "-u 1 --agama-conf-file MWpotentialHunter24_rotspiral.ini" -i 1 input 2>/dev/null | \
...
OMP_NUM_THREADS=1 OMP_STACKSIZE=128M petar -u 1 --agama-conf-file MWpotentialHunter24_rotspiral.ini -t 500.0 -o 1.0 -s "$dt_use" input &>output
...
# generate configure file to plot MWPotential map (see petar.external.agama -h for details). Parameter meaning:
...
# use petar.external.agama to generate potential map from Agama potential for each snapshot, which will be used for the movie of the simulation with external potential.
...
# The option '-p data.par' is used to specify the parameter file prefix for the simulation, which is needed to read the information of the external potential from the parameter file (data.par.agama, generated by petar).
