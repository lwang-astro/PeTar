...
When `-w 2` is used, `data.status` contains one output step followed by all particle records of that step. In this case, initialize `petar.Status` with `N_particle=[particle number]` so the embedded particle block can be parsed correctly. For BINARY `data.status`, use `fromfile()`; for ASCII `data.status`, use `loadtxt()`.
...
# status file written by petar with -i 2 and -w 3
status = petar.Status()
status.fromfile('data.status')

# escaper file gathered by petar.data.gether from petar runtime output
esc = petar.SingleEscaper(interrupt_mode='bse', external_mode='none')
esc.fromfile('data.esc')

# status file written by petar with -w 2 for N particles per output step
status_snap = petar.Status(N_particle=1024, interrupt_mode='bse', external_mode='none')
status_snap.fromfile('data.status')
```
