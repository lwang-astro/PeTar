...
Subsequently, users can utilize the `petar.init` tool (refer to [Initial Input Data File](#initial-input-data-file)) to convert this data file into the input file following the `petar` style.

### Starting a Simulation

The process of initiating an $N$-body simulation using the `petar` command varies based on whether MPI and OpenMP support are compiled.

The standard procedure to commence an $N$-body simulation is as follows:
```
[OMP_STACKSIZE=128M] [OMP_NUM_THREADS=N_threads] [mpiexec -n N_mpi] petar [options] [particle data filename]
```
In this command, `[mpiexec -n N_mpi]` specifies the number of MPI processors (`N_mpi`), while `OMP_NUM_THREADS=N_threads` indicates the utilization of `N_threads` CPU threads by each MPI processor.
