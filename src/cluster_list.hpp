...
#pragma omp for schedule(dynamic)
        for (PS::S32 i=0; i<n_cluster; i++){
            ...
            // merge groups
            if (n_ptcl==2) group.searchAndMerge(ptcl_local.getPointer(), n_ptcl, rout_);
            else group.searchAndMerge(ptcl_local.getPointer(), n_ptcl, rin_);
            ...
            group.generateList(ptcl_local.getPointer(), n_ptcl, ptcl_artifical[ith], adr_sys_multi_cluster_isolated_.getPointer(n_start), n_group_multi_cluster_isolated_[i], rbin_, rin_, rout_, dt_tree_, id_offset_, n_split_);
        }
    ...
    PS::S32 setGroupMemberPars(Tptree &_bin, 
        ...
        for(int i=0; i<2; i++) {
            if(_bin.member[i]->status!=0) 
                nloc += setGroupMemberPars(*(Tptree*)bin.member[i], _pcm_adr, _adr_ref, &_ptcl_list[nloc], _n_split, _id_offset);
            else {
...
//                    }
        ...
        PS::S32 nbin = setGroupMemberPars(_bin, pcm_adr, _ptcl_list, _n_split, _id_offset, _bin.id, true);
...
//        assert(ptcl_map.size()==0);
        ...
        std::sort(plist_now, plist_now+group_list_reorder_offset, [](auto a, auto b) {return a < b;});
        std::sort(_ptcl_list, _ptcl_list+group_list_reorder_offset, [](auto a, auto b) {return a < b;});
