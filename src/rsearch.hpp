...
template<class Tpsys>
void SetBinaryRsearch(Tpsys & psys, const PS::S32 n_bin, const PS::F64 g_min, const PS::F64 r_def) {
  double gamma = std::pow(1.0/g_min,0.33333);
  for (PS::S32 i=0; i<2*n_bin; i+=2) {
    if (psys[i].r_search<=0||psys[i+i].r_search<=0) {
      double a,ecc;
      PosVel2AxEcc(a,ecc,psys[i].pos, psys[i+1].pos, psys[i].vel, psys[i+1].vel, psys[i].mass, psys[i+1].mass);
      if(a>0) psys[i+1].r_search = psys[i].r_search = a*(1+ecc)*gamma;
      else psys[i+1].r_search = psys[i].r_search = r_def;
    }
  }
}
