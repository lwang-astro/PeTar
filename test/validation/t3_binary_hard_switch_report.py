...
def classify_group(r_group: float, peri: float, apo: float) -> str:
    if r_group > apo:
        return "outside_apo"
    if r_group < peri:
        return "inside_peri"
    return "between_peri_apo"
...
def main() -> int:
    ...
    if len(ic_rows) != 2:
        raise RuntimeError(f"T3 report expects a pure binary IC with 2 rows, got {len(ic_rows)}")
    ...
    for run in runs:
        ...
        r_group = float(args_map.get("--r-group", "nan"))
        r_search_group = float(args_map.get("--r-search-group", "nan"))
        group_regime = classify_group(r_group, orb0["peri"], orb0["apo"])
        rows.append(
            {
                "run_id": run["run_id"],
                "r_group": r_group,
                "r_search_group": r_search_group,
                ...
                "sum_period_error": float(run["metrics"].get("sum_abs_error_period_max", 0.0)),
                "dt_soft_times_max_period_error": float(run["metrics"].get("dt_soft_times_max_abs_error_period_max", 0.0)),
                "t_end": drift["t_end"],
    ...
    rows = sorted(rows, key=lambda x: x["r_group"], reverse=True)
    ...
    for row in rows:
        label = f"{row['run_id']} | r_group={row['r_group']:.3e}"
    ...
    run_rows_html = "\n".join(
        "<tr>"
        f"<td>{html_escape(r['run_id'])}</td><td>{r['r_group']:.6e}</td><td>{r['r_search_group']:.6e}</td><td>{html_escape(r['group_regime'])}</td><td>{r['t_end']:.6e}</td><td>{(r['t_end']/orb0['period']):.2f}</td><td>{r['da_rel_max']:.6e}</td><td>{r['de_abs_max']:.6e}</td><td>{r['sum_period_error']:.6e}</td><td>{r['dt_soft_times_max_period_error']:.6e}</td>"
        "</tr>"
        for r in rows
    )
    ...
    command_rows_html = "\n".join(
        "<tr>"
        f"<td>{html_escape(r['run_id'])}</td><td>{r['r_group']:.6e}</td><td>{html_escape(r['group_regime'])}</td><td><code>{html_escape(r['command'])}</code></td><td><code>{html_escape(r['status_path'])}</code></td><td><code>{html_escape(r['log_path'])}</code></td>"
        "</tr>"
        for r in rows
    )
    ...
    html = f"""<!doctype html>
<html lang=\"en\"><head><meta charset=\"utf-8\"><title>T3 Binary Hard-Switching Report</title>
<style>
...
<h1>T3: Binary Long-term Error under Hermite-SDAR Switching Regimes</h1>
<div class=\"note\">
Goal: with fixed <code>r_out</code> and <code>r_in=0.1*r_out &gt; r_apo</code> (all forces in hard region), compare long-term error accumulation for three group-radius regimes by changing <code>--r-group</code>/<code>--r-search-group</code>: outside apocenter, between pericenter and apocenter, and inside pericenter.
</div>

<h2>Figure 1: |Δa/a0| Evolution vs Time (three switching regimes)</h2>
{svg_multi_plot(semi_time_series, 'T3: |Δa/a0|(t) for three r_group regimes', 't / P_orb', '|Δa/a0|', logx=False, logy=True, logy_floor_from_nonzero_x=True, max_x_ticks=6)}

<h2>Figure 2: |Δe| Evolution vs Time (three switching regimes)</h2>
{svg_multi_plot(ecc_time_series, 'T3: |Δe|(t) for three r_group regimes', 't / P_orb', '|Δe|', logx=False, logy=True, logy_floor_from_nonzero_x=True, max_x_ticks=6)}
...
<li>Minimum covered periods across the three groups = {min_periods:.2f}</li>
</ul>

<h2>Run Summary</h2>
<table><tr><th>run_id</th><th>r_group</th><th>r_search_group</th><th>regime</th><th>t_end</th><th>periods covered</th><th>max |Δa/a0|</th><th>max |Δe|</th><th>sum period max |Error/Total|</th><th>dt_soft * max period error</th></tr>
{run_rows_html}
</table>
...
<table><tr><th>run_id</th><th>r_group</th><th>regime</th><th>exact command</th><th>status path</th><th>log path</th></tr>
{command_rows_html}
</table>
...
<li>Two growth proxies are extracted from logs with 100-period binning: <code>sum_abs_error_period_max</code> and <code>dt_soft_times_max_abs_error_period_max</code>.</li>
<li>This test fixes <code>r_out</code> and <code>dt_soft</code>; only group radii differ to isolate Hermite-SDAR switching impact on long-term error accumulation.</li>
</ul>
