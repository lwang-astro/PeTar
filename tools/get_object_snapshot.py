...
if __name__ == '__main__':
    ...
    def usage():
        print("A tool to gether objects from a list of snapshots into one file. Only work when interrupt-mode is bse based code (bse, mobse)")
        ...
        print("Usage: petar.get.object.snap [options] (type|id|id_filename) [typeb] data_filename")
        ...
        print("  -m(--mode): type: select objects based on SSE/MOBSE stellar types; id: select objects based on id; idfile: select objects by reading a file of id list (type)")
    ...
    def select_id(_data, _idlist):
        sel = np.in1d(_data.id, _idlist)
        return sel
            ...
            if (snap_type=='binary'):
                ...
                if os.path.getsize(path+'.binary')>0:
                    ...
                    if (mode=='type'):
                        ...
                        sel1 = select_id(data_temp.p1, idlist)
                        sel2 = select_id(data_temp.p2, idlist)
                        sel = sel1 | sel2
                    read_flag=True
                ...
                if (read_flag):
                    if (mode=='type'):
                        ...
                        sel = select_id(data_temp, idlist)
